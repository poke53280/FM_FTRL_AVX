...
cdef extern from "SingleUpdate.h" nogil:
...
cdef extern from "SinglePredict.h" nogil:
	...
	def predict_f(self, np.ndarray[double, ndim=1, mode='c'] X_data,
		...
		for row in range(row_count):
			...
			if self.use_baseline == 1:
				...
				pp[row]= inv_link_f(predict_single_EXT(inds, vals, lenn,
											   L1, baL2, ialpha, beta, w, z, n,
											   w_fm, z_fm, n_fm, weight_fm,
											   D_fm, bias_term, threads), self.inv_link)
		...
		for iter in range(self.iters):
			...
			for row in range(row_count):
				...
				if self.use_baseline == 1:
					...
					e= inv_link_f(predict_single_EXT(inds, vals, lenn,
											L1, baL2, ialpha, beta, w, z, n,
											w_fm, z_fm, n_fm, weight_fm,
											D_fm, bias_term, threads), inv_link) -ys[row]
